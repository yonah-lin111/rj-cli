...
test("buildResourceMatchCommandPrompt 在单个 Mega 场景中要求先 ask 再更新 source", () => {
  ...
  assert.equal(submission.displayText, "/matchMega [RJ123456]");
  ...
  assert.match(submission.promptText, /单个 RJ 命中时，先明确说明“数据库当前来源\(current_source\)”和“本次匹配来源\(mega\)”/);
  assert.match(submission.promptText, /将 source 设置为 mega/);
  assert.doesNotMatch(submission.promptText, /未经确认不得调用 rj_set_source/);
  assert.match(submission.promptText, /如果返回 message 且 total=0，直接输出“没有匹配项”/);
  assert.match(submission.promptText, /如果 matched=0，只输出“没有匹配项”/);
  assert.match(submission.promptText, /接着输出摘要：检查总数、命中数、失败数/);
  assert.match(submission.promptText, /批量时仅输出命中列表/);
  assert.match(submission.promptText, /不要显示未命中 RJ/);
...
test("buildResourceMatchCommandPrompt 在批量 ASMROne 场景中要求批量确认与 todowrite", () => {
  ...
  assert.equal(submission.displayText, "/matchASMROne []");
  assert.match(submission.promptText, /先且只调用一次 match_asmrone_resources/);
  assert.match(submission.promptText, /批量命中时，先按命中项逐个列出“数据库当前来源\(current_source\)”和“本次匹配来源\(asmrone\)”/);
  assert.match(submission.promptText, /批量流程进入多步骤处理时，再使用 todowrite/);
  assert.match(submission.promptText, /只有在用户明确确认后，才能对确认范围内且确实需要变更的 RJ 调用 rj_set_source，并将 source 设置为 asmrone/);
});
